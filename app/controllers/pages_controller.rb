class PagesController < BaseController
  ...
  def create
    begin
      ...
      @page = @user ? @user.pages.new(params[:page]) : (user_signed_in? ? current_user.pages.new(params[:page]) : Page.new(params[:page]))
    ...
    if @page.save
      redirect_to page_path(:id=>@page.fb_id)
    else
      render :new
    end
  end
  ...
  def update
    respond_to do |format|
      format.js {
        if params[:page][:fan_gate]
          @page.fan_gate = params[:page][:fan_gate]
        end
        @page.save!
      }
    end
